...
use Comely\IO\Emails\Exception\SMTPException;
 ...
 */
class SMTP implements AgentInterface
{
    const EOL  =   "\r\n";
    ...
    {
        fwrite($this->stream, $command . self::EOL);
    }
    ...
    public function send(Message $message, array $emails) : int
    {
        ...
        $this->command(sprintf('MAIL FROM:<%1$s>', $message->getSender()[0]), null, 250); // Set mail from
        $count  =   0;
